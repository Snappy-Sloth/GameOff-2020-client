...
class MixButton extends h2d.Layers {
	...
	public function new(str:String, onClick:Void->Void) {
		...
		var text = new h2d.Text(Assets.fontM5x7gold16);
		text.text = str;
		text.textColor = 0xc6d7d4;
		...
		inter.onReleaseOutside = inter.onOut = function (e) {
			text.x = ((wid/2)-(text.textWidth/2));
			text.y = (hei/2)-(text.textHeight/2);
			spr.set(idSpr + "Idle");
		}
		inter.onPush = function (e) {
			text.x = ((wid/2)-(text.textWidth/2)) + 1;
			text.y = (hei/2)-(text.textHeight/2) + 1;
			spr.set(idSpr + "Press");
